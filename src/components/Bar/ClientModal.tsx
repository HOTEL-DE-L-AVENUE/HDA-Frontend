    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Nouveau Client" size="lg">
      <div className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Input label="Nom" value={form.nom} onChange={e => setForm({...form, nom: e.target.value})} placeholder="Nom" />
          <Input label="Prénom" value={form.prenom} onChange={e => setForm({...form, prenom: e.target.value})} placeholder="Prénom" />
        </div>
        <Input label="Téléphone" value={form.telephone} onChange={e => setForm({...form, telephone: e.target.value})} placeholder="+261 34 12 345 67" />
        ...
        <div className="flex gap-3 pt-2">
          ...
          <Button onClick={handleSubmit} className="flex-1" disabled={!form.nom || !form.prenom}>Enregistrer</Button>
        </div>
      </div>
    </Modal>
  );
