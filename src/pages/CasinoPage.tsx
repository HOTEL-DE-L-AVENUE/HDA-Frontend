...
import { CHIP_VALUES, CasinoView, ChipLine, PlayerLine, casinoBorder, createPlayerLine } from '../components/Casino/sheets/types';
...
export const CasinoPage: React.FC = () => {
  ...
  const [finals, setFinals] = useState<Record<string, string>>({ prelevements: '', pourboires: '', autres: '', restaurant: '', tpe: '', mobiles: '', bonus: '', total1: '', total2: '', difference: '', especes: '', resultat: '', signature: '' });

  const cavesTotal = useMemo(() => players.reduce((total, line) => total + (Number(line.total) || 0), 0), [players]);
  ...
  const savePlayerSheet = async () => {
    setSaveState('saving');
    try {
      await playerSheetApi.save({ date, table_name: table, players, restaurantPayments });
      setSaveState('saved');
    } catch {
      setSaveState('error');
    }
  ...
  return <div className="flex flex-col gap-5 w-full">
    ...
    <main className="rounded-2xl overflow-hidden" style={{ backgroundColor: 'var(--color-surface)', ...casinoBorder }}>
      ...
      <div className="p-4 md:p-5"><div className="hidden print:block text-center mb-5"><h1>{table}</h1><p>Date : {date}</p></div>
        {view === 'players' && <PlayersSheet date={date} players={players} total={cavesTotal} restaurantPayments={restaurantPayments} saveState={saveState} onDateChange={(value) => { setSaveState('idle'); setDate(value); }} onUpdate={(id, key, value) => { setSaveState('idle'); setPlayers((lines) => lines.map((line) => line.id === id ? { ...line, [key]: value } : line)); }} onPaymentChange={(payment, checked) => { setSaveState('idle'); setRestaurantPayments((current) => ({ ...current, [payment]: checked })); }} onSave={savePlayerSheet} onAdd={(ficheId) => { const firstId = Math.max(0, ...players.map((line) => line.id)) + 1; const newFicheId = ficheId ?? firstId; const newLines = ficheId ? [createPlayerLine(firstId, newFicheId)] : Array.from({ length: 5 }, (_, index) => createPlayerLine(firstId + index, newFicheId)); setPlayers((lines) => [...lines, ...newLines]); return newFicheId; }} onRemove={(id) => setPlayers((lines) => lines.filter((line) => line.id !== id))} />}
        {view === 'chips' && <ChipsSheet chips={chips} openingTotal={openingTotal} closingTotal={closingTotal} onUpdate={(value, key, content) => setChips((lines) => lines.map((line) => line.value === value ? { ...line, [key]: content } : line))} />}
        {view === 'final' && <FinalCalculationSheet values={finals} onUpdate={(key, value) => setFinals((current) => ({ ...current, [key]: value }))} />}
      </div>
    </main>
