  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-white text-2xl font-bold" style={{ fontFamily: 'Playfair Display, serif' }}>Finances</h2>
          <p className="text-slate-500 text-sm mt-1">Vue consolidée de toutes les caisses</p>
        </div>
        <div className="flex items-center gap-3">
          <button className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-800 border border-slate-700/50 text-slate-300 hover:text-white text-sm transition-all">
            <Download size={16} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="relative overflow-hidden bg-gradient-to-br from-amber-500/10 to-orange-500/10 border border-amber-500/20 rounded-2xl p-6">
          <div className="absolute top-0 right-0 w-24 h-24 rounded-full blur-2xl bg-amber-500/20" />
          <p className="text-slate-400 text-sm mb-1">Solde Global</p>
          <p className="text-white font-black text-4xl">{formatCurrency(globalStats.soldeGlobal)}</p>
          <div className="flex items-center gap-1 text-amber-400 text-sm mt-2">
            <TrendingUp size={14} />
            <span>+18.4% vs mois dernier</span>
          </div>
        </div>
        <div className="relative overflow-hidden bg-gradient-to-br from-emerald-500/10 to-green-500/10 border border-emerald-500/20 rounded-2xl p-6">
          <div className="absolute top-0 right-0 w-24 h-24 rounded-full blur-2xl bg-emerald-500/20" />
          <p className="text-slate-400 text-sm mb-1">Total Revenus</p>
          <p className="text-emerald-400 font-black text-4xl">{formatCurrency(totalEntrees)}</p>
        ...
        <div className="relative overflow-hidden bg-gradient-to-br from-red-500/10 to-rose-500/10 border border-red-500/20 rounded-2xl p-6">
          <div className="absolute top-0 right-0 w-24 h-24 rounded-full blur-2xl bg-red-500/20" />
          <p className="text-slate-400 text-sm mb-1">Total Dépenses</p>
          <p className="text-red-400 font-black text-4xl">{formatCurrency(totalSorties)}</p>
      ...
      {/* Caisses par Module */}
      <div>
        <h3 className="text-white font-semibold mb-4">Caisses par Module</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
          {modulesSoldes.map(m => {
            const config = moduleConfig[m.module];
            const pct = totalEntrees > 0 ? (m.entrees / totalEntrees) * 100 : 0;
            return (
              <div key={m.module} className="bg-slate-900 border border-slate-800/50 rounded-2xl overflow-hidden hover:border-slate-700/50 transition-all">
                ...
                <div className="p-4 space-y-2">
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="text-emerald-400 font-semibold">{formatCurrency(m.entrees)}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-slate-500">Sorties</span>
                    <span className="text-red-400 font-semibold">{formatCurrency(m.sorties)}</span>
                  </div>
                  <div className="h-1.5 rounded-full bg-slate-800 mt-2">
                    <div className={`h-full rounded-full bg-gradient-to-r ${config.gradient}`} style={{ width: `${pct}%` }} />
                  </div>
                </div>
              </div>
            );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-slate-900 border border-slate-800/50 rounded-2xl p-6">
          <h3 className="text-white font-semibold mb-6">Entrées vs Sorties par Module</h3>
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={barData} barSize={20}>
              ...
              <Bar dataKey="entrees" fill="#10b981" radius={[3, 3, 0, 0]} name="entrees" />
              <Bar dataKey="sorties" fill="#ef4444" radius={[3, 3, 0, 0]} name="sorties" />
            </BarChart>
          </ResponsiveContainer>
        </div>

        <div className="bg-slate-900 border border-slate-800/50 rounded-2xl p-6">
          <h3 className="text-white font-semibold mb-4">Répartition des Revenus</h3>
      ...
      <div className="bg-slate-900 border border-slate-800/50 rounded-2xl overflow-hidden">
          ...
          <div className="flex flex-wrap gap-2">
            {[{ value: 'all', label: 'Tout' }, ...Object.entries(moduleConfig).map(([k, v]) => ({ value: k, label: v.label }))].map(f => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${activeFilter === f.value ? 'bg-amber-500 text-slate-950' : 'bg-slate-800 text-slate-400 hover:text-white'}`}
              >
                {f.label}
              </button>
            ))}
        ...
        <div className="divide-y divide-slate-800/30 max-h-96 overflow-y-auto">
          {allTransactions.map(tx => (
            <div key={tx.id} className="flex items-center gap-4 px-6 py-4 hover:bg-slate-800/20">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${tx.type === 'entree' ? 'bg-emerald-500/15' : 'bg-red-500/15'}`}>
              ...
              <div className={`font-bold whitespace-nowrap ${tx.type === 'entree' ? 'text-emerald-400' : 'text-red-400'}`}>
